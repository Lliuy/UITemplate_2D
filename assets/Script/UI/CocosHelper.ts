...
export default class CocosHelper {
	...
	public static loadResSync<T> (url: string, type: typeof cc.Asset, onProgress?: (completedCount: number, totalCount: number, item: any) => void): Promise<T> {
		return new Promise(async (resolve, reject) => {
			...
			if (!CocosHelper.bundle) {
				CocosHelper.bundle = await CocosHelper.loadBundleSync('Resource', null);
			}
			CocosHelper.bundle.load(url, (err: Error, prefab: any) => {
				if (err) {
					console.error(err);
					resolve(null);
				} else {
	 ...
	 * 播放Animaiton动画
	 */
	public static playAnimation (node: cc.Node, name: string, loop = false): Promise<boolean> {
		return new Promise<boolean>((resolve) => {
			let anim = node.getComponent(cc.Animation);
			if (!anim) {
				cc.warn(`节点没有动画组件, node:${node.name}`);
				resolve(false);
				return;
			}
			let isExit = false;
			for (const clip of anim.getClips()) {
			...
			if (!isExit) {
				cc.warn('没有找到动画', name, 'node:', node.name);
				resolve(false);
				return;
			}
			anim.play(name);
	...
	public static captureScreen (camera: cc.Camera, prop?: cc.Node | cc.Rect) {
		...
		newTexture.initWithSize(cc.visibleRect.width, cc.visibleRect.height, (cc.game as any)._renderContext.STENCIL_INDEX8);
	...
	public static angleToRadian (angle: number) {
		return cc.misc.degreesToRadians(angle);
	}

	/**  动态进度条 */
	public static dynamicProgress (progress: cc.ProgressBar, start: number, end: number) {
		if (progress) {
		}
	}
}
